...
usage() {
  cat <<'USAGE'
Usage: windows-build-tauri-release.sh [options]
...
Examples:
  script/windows-build-tauri-release.sh
  script/windows-build-tauri-release.sh --target x86_64-pc-windows-msvc --bundles msi
  script/windows-build-tauri-release.sh --profile debug --skip-board

Notes:
  * Run from Windows with MSVC toolchain available.
  * Requires Visual Studio Build Tools or equivalent.
  * See .github/workflows/desktop-windows.yml for GitHub Actions setup.
USAGE
}
...
log() { echo "[windows-build-tauri-release] $*"; }

require_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "[windows-build-tauri-release] missing required command: $1" >&2
    exit 1
  fi
}
...
if [[ ! -f "$BOARD_DIR/package.json" ]]; then
  echo "[windows-build-tauri-release] unable to locate board/package.json at $BOARD_DIR" >&2
  exit 1
fi
...
if [[ $BUILD_BOARD -eq 1 ]]; then
  ...
  if [[ -x "$LICENSE_SCRIPT" ]]; then
    ...
    echo "[windows-build-tauri-release] missing license generator at $LICENSE_SCRIPT" >&2
    exit 1
  fi
else
...
build_bridge_sidecar() {
  ...
  local built_path="$BACKEND_DIR/target/$target/$build_subdir/bridge.exe"
  if [[ ! -f "$built_path" ]]; then
    echo "[windows-build-tauri-release] missing bridge binary at $built_path" >&2
    exit 1
  fi
  cp "$built_path" "$SIDECAR_OUTPUT_DIR/bridge-$target.exe"
  cp "$built_path" "$SIDECAR_OUTPUT_DIR/bridge.exe"
}
...
build_core_sidecar() {
  ...
  local built_path="$BACKEND_DIR/target/$target/$build_subdir/mcpmate.exe"
  if [[ ! -f "$built_path" ]]; then
    echo "[windows-build-tauri-release] missing core binary at $built_path" >&2
    exit 1
  fi
  cp "$built_path" "$SIDECAR_OUTPUT_DIR/mcpmate-core-$target.exe"
...
if [[ "$PROFILE" == "debug" ]]; then
  ...
  echo "[windows-build-tauri-release] unsupported profile: $PROFILE" >&2
  exit 1
fi
...
bundle_dir="$ROOT_DIR/src-tauri/target/$TARGET/$PROFILE/bundle/msi"

declare -a artifacts=()
if [[ -d "$bundle_dir" ]]; then
  while IFS= read -r -d '' file; do
    artifacts+=("$file")
  done < <(find "$bundle_dir" -maxdepth 1 -type f -name "*.msi" -print0 2>/dev/null) || true
fi

if [[ ${#artifacts[@]} -eq 0 ]]; then
  echo "[windows-build-tauri-release] warning: no MSI artifacts found in $bundle_dir" >&2
  exit 1
fi
