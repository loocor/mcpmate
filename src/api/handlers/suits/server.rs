...
pub async fn enable_server(
    ...
    crate::conf::operations::add_server_to_config_suit(&db.pool, &suit_id, &server_id, true)
        .await
        .map_err(|e| {
...
pub async fn disable_server(
    ...
    crate::conf::operations::add_server_to_config_suit(&db.pool, &suit_id, &server_id, false)
        .await
        .map_err(|e| {
            ApiError::InternalError(format!(
...
pub async fn batch_enable_servers(
    ...
    for server_id in payload.ids {
        ...
        match server {
            Some(_) => {
                // Enable the server in the suit
                match crate::conf::operations::add_server_to_config_suit(
