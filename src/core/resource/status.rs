...
pub async fn is_resource_enabled(
    ...
    for suit in active_suits {
        if let Some(suit_id) = &suit.id {
            ...
            let enabled_resources = crate::config::suit::get_enabled_resources_for_config_suit(pool, suit_id)
                .await
                .context(format!("Failed to get enabled resources for suit '{suit_id}'"))?;
