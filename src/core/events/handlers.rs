...
pub type ServerSyncCallback = Arc<dyn Fn() -> BoxFuture<'static, CoreResult<()>> + Send + Sync>;
...
impl EventHandlers {
    ...
    pub fn register_server_sync_callback<F>(
        ...
        F: Fn() -> BoxFuture<'static, CoreResult<()>> + Send + Sync + 'static,
    {
        self.server_sync_callback = Some(Arc::new(callback));
        info!("Registered server sync callback for core events");
    }
