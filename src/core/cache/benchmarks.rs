...
use super::{manager::RedbCacheManager, types::*};
...
impl CacheBenchmarker {
    ...
    /// Run comprehensive benchmark profile
    pub async fn run_benchmark_profile(&self) -> Result<BenchmarkResults, CacheError> {
    ...
    async fn benchmark_write_operations(
        &self,
        test_data: &[CachedServerData],
    ) -> Result<OperationBenchmark, CacheError> {
    ...
    async fn benchmark_concurrent_operations(
        &self,
        test_data: &[CachedServerData],
    ) -> Result<ConcurrentBenchmark, CacheError> {
        // Concurrent reads
    ...
    async fn benchmark_concurrent_writes(
        &self,
        test_data: &[CachedServerData],
    ) -> Result<OperationBenchmark, CacheError> {
        let mut durations = Vec::new();
        ...
        for _ in 0..self.config.benchmark_iterations {
            ...
            for chunk in test_data.chunks(self.config.concurrent_operations) {
                for server_data in chunk {
                    ...
                    let handle = tokio::spawn(async move { cache_manager.store_server_data(&server_data).await });
    ...
    async fn benchmark_mixed_operations(
        ...
        for _ in 0..self.config.benchmark_iterations {
            ...
            for (i, server_data) in test_data.iter().enumerate() {
                ...
                let handle = if i % 2 == 0 {
                    // Write operation
                    tokio::spawn(async move { cache_manager.store_server_data(&server_data).await.map(|_| ()) })
                } else {
    ...
    async fn calculate_storage_efficiency(
        &self,
        test_data: &[CachedServerData],
    ) -> Result<StorageEfficiency, CacheError> {
        // Store all test data
        for server_data in test_data {
            self.cache_manager.store_server_data(server_data).await?;
        }
    ...
    fn calculate_operation_benchmark(
        &self,
    ...
    fn estimate_raw_data_size(
        ...
        test_data
            .iter()
            .map(|data| {
                let tools_size: usize = data
                    .tools
                    .iter()
                    .map(|t| t.name.len() + t.description.as_ref().map_or(0, |d| d.len()))
                    .sum();
                let resources_size: usize = data
                    .resources
                    .iter()
                    .map(|r| r.uri.len() + r.name.as_ref().map_or(0, |n| n.len()))
                    .sum();
                let prompts_size: usize = data
                    .prompts
                    .iter()
                    .map(|p| p.name.len() + p.description.as_ref().map_or(0, |d| d.len()))
                    .sum();
...
pub fn print_benchmark_results(results: &BenchmarkResults) {
    ...
    println!(
        ...
        results.storage_efficiency.total_cache_size_bytes as f64 / 1024.0 / 1024.0
    );
    println!(
        "Average Item Size: {:.2} bytes",
        results.storage_efficiency.average_item_size_bytes
    );
    println!(
        "Compression Ratio: {:.2}x",
        results.storage_efficiency.compression_ratio
    );
    println!(
        "Storage Overhead: {:.2}%",
        results.storage_efficiency.storage_overhead_percent
    );
}
