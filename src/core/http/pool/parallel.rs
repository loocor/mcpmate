// Parallel connection management for UpstreamConnectionPool
// Provides high-performance parallel connection capabilities while maintaining
// full compatibility with existing lifecycle management
...
impl ParallelConnectionManager {
    ...
    async fn perform_connection(
        ...
        let server_config = config
            .mcp_servers
            .get(server_name)
            .ok_or_else(|| anyhow::anyhow!("Server configuration not found"))?;
        ...
        match server_type {
            ...
            ServerType::StreamableHttp => {
                ...
                Self::perform_http_connection(server_name, server_config, transport_type).await
            }
        }
    }

    /// Stdio connection logic (extracted from existing code)
    async fn perform_stdio_connection(
        ...
        // Store cancellation token via event
        ...
        // Use existing connection functions
        let connect_result = if let Some(runtime_cache) = runtime_cache {
    ...
    async fn perform_http_connection(
        ...
        use crate::core::{connect_http_server, connect_sse_server, transport::TransportType};

        let server_type = server_config.kind;

        // Wait for transport ready if needed
        ...
        connect_result.map(|(service, tools, capabilities)| {
            (service, tools, capabilities, None) // HTTP connections don't have process ID
        })
    }

    /// Centralized state management (preserves all existing lifecycle capabilities)
        ...
        while let Some(event) = event_rx.recv().await {
            ...
            match event {
                ConnectionEvent::Connecting {
                    ...
                    pool.log_connection_event(
                        tracing::Level::INFO,
                        &server_name,
                        &instance_id,
                        "Starting parallel connection to",
                    );
                }
                ConnectionEvent::Connected {
                    ...
                    // Reuse existing update_connection method
                    ...
                    if let (Some(pid), Ok(conn)) = (
                        process_id,
                        pool.get_instance_mut(&server_name, &instance_id),
                    ) {
                ...
                ConnectionEvent::Failed {
                    ...
                    if let Ok(conn) = pool.get_instance_mut(&server_name, &instance_id) {
                        conn.update_failed(error.clone());
                    }
                    pool.log_connection_event(
                        tracing::Level::ERROR,
                        &server_name,
                        &instance_id,
                        &format!("Parallel connection failed: {}", error),
                    );
                }
...
/// Extension methods for UpstreamConnectionPool to support parallel connections
impl UpstreamConnectionPool {
    ...
    /// Trigger parallel connection to all servers (new high-performance method)
