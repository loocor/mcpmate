...
///
/// This function checks if a prompt is enabled by looking at all active configuration suits.
/// A prompt is considered enabled if it's enabled in at least one active configuration suit.
///
/// # Arguments
/// * `pool` - Database connection pool
/// * `server_name` - Name of the server providing the prompt
/// * `prompt_name` - Name of the prompt to check
///
/// # Returns
/// * `Ok(true)` if the prompt is enabled in at least one active configuration suit
/// * `Ok(false)` if the prompt is not enabled in any active configuration suit
...
pub async fn is_prompt_enabled(
    ...
    for suit in active_suits {
        if let Some(suit_id) = &suit.id {
            ...
            let enabled_prompts = crate::config::suit::get_enabled_prompts_for_config_suit(pool, suit_id)
                .await
                .context(format!("Failed to get enabled prompts for suit '{suit_id}'"))?;
...
///
/// This function returns the prompt ID and enabled status for a prompt.
/// It's used by API handlers to get prompt information.
///
/// # Arguments
/// * `pool` - Database connection pool
/// * `server_name` - Name of the server providing the prompt
...
///
/// # Returns
/// * `Ok((prompt_id, enabled))` if the prompt is found
/// * `Err(_)` if the prompt is not found or there was a database error
