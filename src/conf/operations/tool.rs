...
/// Update the prefixed name of a tool in the database
pub async fn update_tool_prefixed_name(
    pool: &Pool<Sqlite>,
    server_name: &str,
    tool_name: &str,
    ...
    if let Some(server) = server {
        if let Some(server_id) = &server.id {
            ...
            if active_suits.is_empty() {
                ...
                // Update the tool's prefixed name in the config suit
                let tool_id = update_tool_prefixed_name_in_suit(
                    pool,
                    &suit_id,
                    server_id,
                    tool_name,
...
/// Helper function to update a tool's prefixed name in a specific suit
async fn update_tool_prefixed_name_in_suit(
    pool: &Pool<Sqlite>,
    config_suit_id: &str,
    server_id: &str,
    tool_name: &str,
    ...
    // Check if the tool exists in the suit
    let existing_tool = sqlx::query_as::<_, crate::conf::models::ConfigSuitTool>(
        r#"
...
pub async fn get_tool_prefixed_name(
    pool: &Pool<Sqlite>,
    server_name: &str,
    tool_name: &str,
) -> Result<Option<String>> {
