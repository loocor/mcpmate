...
#[cfg(feature = "warp-sqlite")]
pub struct WarpSqliteStorage { config_source: Arc<dyn ClientConfigSource> }

#[cfg(feature = "warp-sqlite")]
impl WarpSqliteStorage {
    pub fn new(config_source: Arc<dyn ClientConfigSource>) -> Self { Self { config_source } }

    async fn resolve_db_path(&self, template: &ClientTemplate) -> ConfigResult<std::path::PathBuf> {
        ...
        let raw = self
            ...
            .await?
            .ok_or_else(|| ConfigError::PathResolutionError(format!("Failed to resolve Warp DB path for {}", template.identifier)))?;
        crate::system::paths::get_path_service().resolve_user_path(&raw).map_err(|e| ConfigError::PathResolutionError(e.to_string()))
    }
    ...
    async fn open_pool(path: &std::path::Path) -> ConfigResult<SqlitePool> {
        ...
        SqlitePool::connect_with(opts).await.map_err(|e| ConfigError::DataAccessError(e.to_string()))
    }
}
...
impl ConfigStorage for WarpSqliteStorage {
    fn kind(&self) -> StorageKind { StorageKind::Custom }

    async fn read(&self, template: &ClientTemplate) -> ConfigResult<Option<String>> {
        let path = match self.resolve_db_path(template).await { Ok(p) => p, Err(ConfigError::PathResolutionError(_)) => return Ok(None), Err(e) => return Err(e) };
        ...
        .fetch_all(&pool)
        .await
        .map_err(|e| ConfigError::FileOperationError(format!("Warp storage read failed (possible version change): {}", e)))?;
        ...
        for r in rows {
            ...
            if let Ok(Some(env_s)) = r.try_get::<Option<String>, _>("env_json") {
                if let Ok(env_v) = serde_json::from_str::<serde_json::Value>(&env_s) {
                    if let Some(obj) = v.as_object_mut() { obj.insert("env".into(), env_v); }
                }
            }
            list.push(v);
        }
        let value = serde_json::json!({"servers": list});
        Ok(Some(serde_json::to_string_pretty(&value).map_err(ConfigError::JsonError)?))
    }

    async fn write_atomic(&self, template: &ClientTemplate, content: &str, policy: &BackupPolicySetting) -> ConfigResult<Option<String>> {
        let path = self.resolve_db_path(template).await?;
        let pool = Self::open_pool(&path).await?;
        let mut tx = pool.begin().await.map_err(|e| ConfigError::FileOperationError(format!("Warp storage write failed to begin tx: {}", e)))?;
        ...
        let _ = sqlx::query("delete from mcp_environment_variables").execute(&mut *tx).await;
        sqlx::query("delete from object_metadata where object_type='GENERIC_STRING_JSON_MCPSERVER'")
            .execute(&mut *tx).await.map_err(|e| ConfigError::FileOperationError(format!("Warp storage write failed (schema?): {}", e)))?;
        // Clean up unreferenced rows (best-effort)
        sqlx::query("delete from generic_string_objects where id not in (select shareable_object_id from object_metadata)")
            .execute(&mut *tx).await.map_err(|e| ConfigError::FileOperationError(format!("Warp storage cleanup failed: {}", e)))?;

        let doc: serde_json::Value = serde_json::from_str(content).map_err(ConfigError::JsonError)?;
        let servers = doc.get("servers").and_then(|v| v.as_array()).cloned().unwrap_or_default();
        for mut v in servers {
            // Ensure stable uuid field
            let uuid_text = v.get("uuid").and_then(|x| x.as_str()).and_then(|s| Uuid::parse_str(s).ok()).map(|u| u.to_string())
                .or_else(|| {
            ...
            let row = sqlx::query("select last_insert_rowid() as id").fetch_one(&mut *tx).await.map_err(|e| ConfigError::FileOperationError(format!("Warp storage last_rowid failed: {}", e)))?;
            ...
            let uuid = Uuid::parse_str(&uuid_text).map_err(|e| ConfigError::FileOperationError(format!("Warp uuid parse failed: {}", e)))?;
        ...
        tx.commit().await.map_err(|e| ConfigError::FileOperationError(format!("Warp storage commit failed: {}", e)))?;
    ...
    async fn list_backups(&self, template: &ClientTemplate) -> ConfigResult<Vec<BackupFile>> {
        let path = self.resolve_db_path(template).await?;
    ...
    async fn delete_backup(&self, template: &ClientTemplate, backup_name: &str) -> ConfigResult<()> {
        let path = self.resolve_db_path(template).await?;
        let p = path.parent().unwrap_or_else(|| std::path::Path::new(".")).join(backup_name);
        if p.exists() { std::fs::remove_file(p).map_err(|e| ConfigError::FileOperationError(e.to_string()))?; }
        Ok(())
    }

    async fn restore_backup(&self, template: &ClientTemplate, backup_name: &str, _policy: &BackupPolicySetting) -> ConfigResult<Option<String>> {
        let path = self.resolve_db_path(template).await?;
        let p = path.parent().unwrap_or_else(|| std::path::Path::new(".")).join(backup_name);
        if !p.exists() { return Err(ConfigError::FileOperationError(format!("Backup {} not found", backup_name))); }
...
#[cfg(feature = "warp-sqlite")]
impl WarpSqliteStorage { pub fn as_dyn(self) -> DynConfigStorage { Arc::new(self) } }
