//! Initialization logic for recore proxy server
//!
//! This module handles the setup and initialization of the proxy server using recore modules.
...
use crate::{
    ...
    recore::{events, foundation::loader},
    runtime::migration,
};
...
/// Setup proxy server with database and configuration using recore modules
pub async fn setup_proxy_server(db: Database) -> Result<(ProxyServer, Arc<ProxyServer>)> {
    // Load configuration from database using recore loader
    ...
    // Create proxy server using recore implementation
    ...
    // Initialize the event system using recore
    let _ = events::init();
    tracing::info!("Event system initialized using recore");
