//! Database configuration loader for recore MCPMate
...
use crate::{
    ...
    recore::models::{Config, MCPServerConfig},
};

/// Load the MCP server configuration from the database
pub async fn load_server_config(db: &Database) -> Result<Config> {
    tracing::info!("Loading server configuration from database using recore loader");
    ...
    tracing::info!(
        "Successfully loaded {} enabled servers from database using recore loader",
        config.mcp_servers.len()
    );

    // Publish ConfigReloaded event using recore events
    crate::recore::events::EventBus::global().publish(crate::recore::events::Event::ConfigReloaded);
    tracing::info!("Published ConfigReloaded event using recore events");
