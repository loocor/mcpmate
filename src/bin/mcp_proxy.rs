...
use clap::Parser;
use mcp_client::{
    config::{load_rule_config, load_server_config, Config},
    proxy,
};
use rmcp::{
...
impl UpstreamConnectionPool {
    ...
    async fn connect_stdio(&mut self, server_name: &str) -> Result<()> {
        ...
        match proxy::connect_stdio_server(server_name, server_config).await {
            Ok((service, tools)) => {
                // Update connection
                let conn = self.connections.get_mut(server_name).unwrap();
                conn.tools = tools;
    ...
    async fn connect_sse(&mut self, server_name: &str) -> Result<()> {
        ...
        // Connect to the server using the proxy module function
        match proxy::connect_sse_server(server_name, server_config).await {
            Ok((service, tools)) => {
                // Update connection
                let conn = self.connections.get_mut(server_name).unwrap();
                conn.tools = tools;
    ...
    async fn connect_all(&mut self) -> Result<()> {
        ...
        for name in server_names {
            ...
            join_set.spawn(async move {
                ...
                let result = match server_type.as_str() {
                    "stdio" => {
                        // Use the proxy module function to connect
                        match proxy::connect_stdio_server(&name_clone, server_config).await {
                            Ok((service, tools)) => (name_clone, Ok((service, tools))),
                            Err(e) => (name_clone, Err(e)),
                        }
                    }
                    "sse" => {
                        // Use the proxy module function to connect
                        match proxy::connect_sse_server(&name_clone, server_config).await {
                            Ok((service, tools)) => (name_clone, Ok((service, tools))),
                            Err(e) => (name_clone, Err(e)),
                        }
                    }
