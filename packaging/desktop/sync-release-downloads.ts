...
function requiredTrackedAssets(manifest: unknown): Array<[string, TrackedReleaseAsset]> {
  ...
  return Object.entries(manifest.assets).flatMap(([assetKey, value]) => {
    if (!isRecord(value) || typeof value.githubReleaseUrl !== "string" || typeof value.homebrewUrl !== "string") {
      return [];
    }
    ...
    return [[
      assetKey,
      {
        githubReleaseUrl: value.githubReleaseUrl,
        homebrewUrl: value.homebrewUrl,
...
export async function synchronizeReleaseDownloads(
  ...
  const manifest = validateReleaseManifest(manifestInput, options.tag, publicOrigin);

  for (const [assetKey, asset] of requiredTrackedAssets(manifest)) {
    for (const [route, url] of [
      ["release", asset.githubReleaseUrl],
      ["Homebrew", asset.homebrewUrl],
    ] as const) {
      const redirectResponse = await fetcher(url, { redirect: "manual" });
      if (redirectResponse.status !== 302) {
        throw new Error(
          `${route} download for ${assetKey} must return HTTP 302; received ${redirectResponse.status}`,
        );
      }

      const location = redirectResponse.headers.get("location");
      if (!location) {
        throw new Error(`${route} download for ${assetKey} is missing the Location header`);
      }
      if (location !== asset.githubUrl) {
        throw new Error(`${route} download Location for ${assetKey} does not match manifest githubUrl`);
      }
    }
  }
