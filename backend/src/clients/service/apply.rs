    ...
    pub async fn apply_with_deferred(
        ...
        match self.execute_render(options.clone()).await {
            ...
            Err(ConfigError::FileOperationError(msg)) if msg.to_ascii_lowercase().contains("locked") => {
                let state = self.fetch_state(&options.client_id).await?.ok_or_else(|| {
                    ConfigError::DataAccessError(format!("Client {} not found", options.client_id))
                })?;
                if self.template_engine.storage_for_client(&state).is_err() {
                ...
                let mut out = preview_outcome;
                out.scheduled = true;
                out.scheduled_reason = Some("target_locked".into());
                Ok(out)
            }
            Err(e) => Err(e),
        }
