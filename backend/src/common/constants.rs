...
pub mod timeouts {
    ...
    /// Retry window when a managed config target remains locked (seconds).
    pub const LOCKED_WRITE_RETRY_WINDOW_SEC: u64 = 120;
    /// Retry interval when a managed config target remains locked (milliseconds).
    pub const LOCKED_WRITE_RETRY_INTERVAL_MS: u64 = 1000;
}
