...
pub trait FingerprintGenerator {
    async fn generate_fingerprint(
        &self,
        server_path: &Path,
    ) -> Result<MCPServerFingerprint, FingerprintError>;
    async fn compare_fingerprints(
        &self,
        old: &MCPServerFingerprint,
        new: &MCPServerFingerprint,
    ) -> FingerprintDiff;
}
...
impl DefaultFingerprintGenerator {
    ...
    async fn generate_code_fingerprint(
        &self,
        server_path: &Path,
    ) -> Result<CodeFingerprint, FingerprintError> {
        ...
        for entry in walkdir::WalkDir::new(server_path).into_iter().filter_map(|e| e.ok()) {
            let path = entry.path();
    ...
    async fn generate_dependency_fingerprint(
        &self,
        server_path: &Path,
    ) -> Result<DependencyFingerprint, FingerprintError> {
        ...
        for (manifest_file, lock_file) in &package_files {
            ...
            if manifest_path.exists() {
                ...
                resolved_versions = self
                    .parse_manifest_versions(&manifest_path, manifest_file)
                    .await
                    .unwrap_or_default();
    ...
    async fn parse_manifest_versions(
        &self,
        manifest_path: &Path,
        file_type: &str,
    ...
    async fn generate_capability_fingerprint(
        &self,
        server_id: &str,
    ) -> Result<CapabilityFingerprint, FingerprintError> {
        ...
        if let Some(data) = server_data {
            ...
            let server_info = format!(
                "{}-{}-{}",
                data.server_name,
                data.server_version.unwrap_or_default(),
    ...
    async fn compare_fingerprints(
        ...
        if old.capability_fingerprint != new.capability_fingerprint {
            let capabilities_diff = CapabilityDiff {
                ...
                resources_changed: old.capability_fingerprint.resources_hash
                    != new.capability_fingerprint.resources_hash,
                prompts_changed: old.capability_fingerprint.prompts_hash != new.capability_fingerprint.prompts_hash,
                server_info_changed: old.capability_fingerprint.server_info_hash
                    != new.capability_fingerprint.server_info_hash,
            };
        ...
        if old.config_fingerprint != new.config_fingerprint {
            let config_diff = ConfigDiff {
                server_config_changed: old.config_fingerprint.server_config_hash
                    != new.config_fingerprint.server_config_hash,
...
impl ChangeDetector {
    ...
    pub async fn detect_changes(
        ...
        let diff = self
            .fingerprint_generator
            .compare_fingerprints(&old_fingerprint, &new_fingerprint)
            .await;
    ...
    async fn get_stored_fingerprint(
        &self,
        _server_id: &str,
    ) -> Result<MCPServerFingerprint, FingerprintError> {
    ...
    async fn store_fingerprint(
        &self,
        server_id: &str,
        _fingerprint: &MCPServerFingerprint,
    ...
    pub async fn invalidate_cache_if_changed(
        &self,
        server_id: &str,
