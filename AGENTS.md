...
- Use `release-alignment` for release description, changelog, README, roadmap, localized website docs, and Cloudflare preview handoff rules around release-facing documentation.
...
- Treat PR readiness as "ready for Loocor/Codex review" by default. Prefer Codex Review over local-only signoff; trigger it with a PR comment `@codex review` when the PR is ready. Keep the PR reviewable and evidence-rich instead of collapsing review and merge into one automated step.
