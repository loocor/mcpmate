...
MCPMate 将 Claude Desktop、Cursor、Codex、Zed 以及其他兼容客户端的 MCP 服务器管理集中到一个本地控制面。

它减少重复配置，通过配置集限制不必要的工具暴露，协调客户端配置变更，并为 MCP 使用提供本地 Dashboard、API 和结构化日志轨迹。

你可以从 Transparent 配置同步开始，逐步进入 Hosted 代理管理，并在需要更强治理时采用 Unify 风格的能力可见性控制。
