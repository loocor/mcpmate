...
test("Workflow Guide notebook documents and preview persist the intended Markdown", async ({ page, request }) => {
  ...
  const initialMarkdown = [
    ...
    "Inspect release notes, then compare linked pull requests.",
    ":::",
  ].join("\n");
  ...
  await expect(page.getByLabel("Workflow Guide notebook")).toContainText("Collect evidence");
  await page.screenshot({ path: "test-results/workflow-guide-production/notebook.png", fullPage: true });
  ...
  await expect(page.getByText("Markdown section")).toBeVisible();
  await expect(page.getByRole("button", { name: "Workflow step" })).toBeVisible();
  await page.screenshot({ path: "test-results/workflow-guide-production/boundary-insert.png", fullPage: true });
  await page.keyboard.press("Escape");

  await page.locator('[id^="guide-cell-workflow-step-"]').filter({ hasText: "Collect evidence" }).getByRole("button", { name: "Edit block" }).click();
  await expect(page.getByText("Step key")).toBeVisible();
  await expect(page.getByLabel("Workflow step instructions")).toHaveValue("Inspect release notes, then compare linked pull requests.");
  await page.screenshot({ path: "test-results/workflow-guide-production/block-edit.png", fullPage: true });
  ...
  await page.getByText("Capabilities").last().click();
  ...
  await page.screenshot({ path: "test-results/workflow-guide-production/capability-insert.png", fullPage: true });
  ...
  await page.screenshot({ path: "test-results/workflow-guide-production/skill-preview.png", fullPage: true });
  ...
  await page.screenshot({ path: "test-results/workflow-guide-production/saved-notebook.png", fullPage: true });
  ...
  await page.getByText("Materials").last().click();
  ...
  await page.screenshot({ path: "test-results/workflow-guide-production/external-notebook.png", fullPage: true });
  ...
  await expect(page.getByRole("region", { name: "Preview" })).toContainText(/Capability: .*collect_evidence/);
  await page.screenshot({ path: "test-results/workflow-guide-production/external-preview.png", fullPage: true });
  ...
  for (const material of [
    ...
    await page.getByLabel("Package file category").selectOption(material.category);
  ...
  await expect(page.getByLabel("Guide inspector")).toContainText("Summarize evidence");
  await expect(page.getByLabel("Guide outline")).toContainText("Release policy");
  await page.screenshot({ path: "test-results/workflow-guide-production/materials-and-inspector.png", fullPage: true });
  ...
  await page.screenshot({ path: "test-results/workflow-guide-production/repaired-skill.png", fullPage: true });
});
