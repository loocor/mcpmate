	...
	return (
		...
		>
			...
			<Callout type="warning" title="キャッシュのリセットはダウンロードしたパッケージを削除します">
				Node.js、uv、Bun のキャッシュをリセットすると、そのランタイムでダウンロード済みのパッケージやキャッシュ内容が削除されます。その後のサーバー呼び出しでは依存関係が再インストールされ、時間がかかる場合があります。エンドユーザーが接続している間ではなく、メンテナンスウィンドウ中または自動テストを実行する前にリセットをスケジュールしてください。
			</Callout>
		</DocLayout>
	);
}
