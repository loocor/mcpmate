...
export default function SettingsZH() {
	return (
		<DocLayout meta={{ title: "设置", description: "应用偏好与配置" }}>
			...
			<Ul>
				<Li>
					<strong>通用</strong>：选择默认列表视图、界面语言（简体中文、English、日本語），并调整行为默认值。
				</Li>
				...
				<Li><strong>客户端默认值</strong>：设置统一模式、托管模式、透明模式、默认过滤器、备份策略与备份上限，影响客户端页面的工具栏与行为。</Li>
			...
			<H3>控制客户端策略</H3>
			<P>
				“客户端默认值”允许设定统一模式、托管模式、透明模式、默认过滤器与备份策略，这些参数会写入全局状态供客户端页面读取，从而保持工具栏行为一致。
			</P>
			...
			<Callout type="info" title="为什么端口配置值得特别说明">
				代码里已经有“端口变更后重写托管模式客户端配置”的专门支持，也有面向透明模式客户端的配置集同步路径。对用户来说，这意味着受管路径本来就是朝着“尽量不要手工回头逐个改客户端配置”这个方向设计的。
			</Callout>
