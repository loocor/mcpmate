...
export default function ClientAppsZH() {
	return (
		<DocLayout meta={{ title: "兼容应用", description: "与 MCPMate 集成的应用" }}>
			...
			<H2>从 Discovery 预设添加客户端</H2>
			<P>
				New / Edit 抽屉可以加载 Public Discovery 中的客户端条目。选择预设后，抽屉会带入已知标识符、显示名称、图标、主页和 MCP 配置目标，你可以在保存前确认管理模式与写入策略。
			</P>
			<Ul>
				<Li>首次引导时，适合用它快速添加已支持的 AI 应用。</Li>
				<Li>自定义客户端也可以通过同一个抽屉声明标准 MCP 配置位置。</Li>
				<Li>保存后，客户端会进入列表，并获得与本地检测应用一致的治理控制。</Li>
			</Ul>
			...
			<Callout type="info" title="只有已验证的本地配置目标才允许写入">
				...
				托管模式、统一模式、透明模式都会复用这份已验证的本地目标来生成写入结果。
			</Callout>
		</DocLayout>
	);
}
