...
export default function ProfileCapabilitiesZH() {
	return (
		<DocLayout meta={{ title: "能力标签页", description: "控制某个配置集真正暴露哪些服务器与能力" }}>
			<P>
				配置集能力管理决定配置集最终向客户端暴露什么。共享能力界面支持搜索、筛选、检视和批量编辑，用来确定配置集激活后哪些服务器、工具、提示词、资源和模板仍然可见。
			</P>
			...
			<H2>通用操作建议</H2>
			<Ul>
				<Li>先搜索或过滤，再批量操作，避免误伤无关条目。</Li>
				<Li>变更配置集暴露面前，如果需要验证真实行为，可从 server 行直接进入 Inspect。</Li>
