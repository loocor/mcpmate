...
export default function ProtocolBridging() {
	return (
		<DocLayout
			meta={{
				title: "协议桥接",
				description: "无需修改客户端即可将基于 stdio 的客户端连接到 Streamable HTTP 服务",
			}}
		>
			<P>
				MCPMate 的协议桥接功能允许基于 stdio 的 MCP 客户端连接到 Streamable
				HTTP 服务，而无需修改客户端代码。这使得部署和使用 MCP
				服务器具有更大的灵活性。
			</P>
			...
			<P>
				...
				时，它可以与 Streamable HTTP 服务器通信，就像它们是原生 stdio
				服务器一样。协议转换在后台无缝进行。历史 SSE 风格配置会在导入边界被兼容接收并归一化为
				Streamable HTTP。
			</P>
