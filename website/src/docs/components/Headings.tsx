...
export const H2: React.FC<React.PropsWithChildren<{ id?: string }>> = ({
	...
	return (
		<h2
			id={resolvedId}
			ref={ref}
			className="scroll-mt-24 text-2xl font-semibold text-brand-foreground flex items-center gap-2"
		>
			<a
				href={`#${resolvedId}`}
				className="opacity-30 group-hover:opacity-100 transition-opacity text-brand-muted-soft hover:text-brand-accent"
			>
				#
			</a>
			<span>{children}</span>
...
export const H3: React.FC<React.PropsWithChildren<{ id?: string }>> = ({
	...
	return (
		<h3
			id={resolvedId}
			ref={ref}
			className="scroll-mt-24 text-xl font-semibold text-brand-foreground flex items-center gap-2"
		>
			<a
				href={`#${resolvedId}`}
				className="opacity-30 group-hover:opacity-100 transition-opacity text-brand-muted-soft hover:text-brand-accent"
...
export const P: React.FC<React.PropsWithChildren> = ({ children }) => (
	<p className="leading-7 text-brand-muted">{children}</p>
);
...
export const Li: React.FC<React.PropsWithChildren> = ({ children }) => (
	<li className="text-brand-muted">{children}</li>
);
